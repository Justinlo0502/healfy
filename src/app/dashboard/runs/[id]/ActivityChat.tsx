...
export default function ActivityChat({ activityId }: { activityId: string }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-line bg-surface shadow-card">
      <div ref={listRef} className="flex max-h-[40vh] min-h-[160px] flex-col gap-3 overflow-y-auto p-4">
        {messages.length === 0 && (
          <p className="text-sm text-muted">Ask anything about this specific run — pace, effort, terrain, weather.</p>
        )}
          ...
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] whitespace-pre-wrap rounded-lg px-3.5 py-2.5 text-sm leading-relaxed ${
                m.role === "user" ? "bg-accent/10 text-foreground" : "bg-surface-2 text-foreground"
              }`}
            >
              {m.content}
            </div>
        ...
        {error && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-lg bg-danger-bg px-3.5 py-2.5 text-sm text-danger">{error}</div>
          </div>
        )}
      </div>
